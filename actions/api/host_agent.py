...
from actions.api.common.client import A2ACardResolver
...
class HostAgent:
  ...
  def __init__(
      ...
      task_callback: Optional[TaskUpdateCallback] = None  # Ensure task_callback is optional
  ):
    self.task_callback = task_callback  # Initialize task_callback
...
"""
  ...
  async def send_task(
      ...
      agent_name: str,
      message: str,
      tool_context: Optional[dict] = None  # Replace ToolContext with a generic dict
  ):
    ...
    Args:
      ...
      tool_context: A dictionary representing the tool context.
    ...
    state = tool_context.get("state", {}) if tool_context else {}
...
def convert_parts(parts: list[Part], tool_context: dict):
  rval = []
  for p in parts:
    rval.append(convert_part(p, tool_context))
  return rval

def convert_part(part: Part, tool_context: dict):
  if part.type == "text":
    return part.text
  elif part.type == "data":
    return part.data
    ...
    tool_context["skip_summarization"] = True
    tool_context["escalate"] = True
